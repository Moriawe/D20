<style scoped>
p {
  text-align: left;
}

li {
  text-align: left;
}
</style>

<template>
  <div class="container-md">
    <h1>FAQ</h1>
    <h3>Den ser ju ut som en bondkatt?</h3>
    <p>
      Ja! Den är vår urgamla, inhemska korthårskatt och direkt ättling till den
      första tamkatten, det är därför den har utsetts till Sveriges nationalras.
      Skillnaden är att en europé har en stamtavla, den är renrasig.
    </p>
    <p>
      Förr i tiden härstammade ALLA tamkatter ifrån två olika vildkattraser, en
      från Turkiet och en ifrån Egypten. Denna blandning bodde med människorna i
      10.000 år och blev den katt de flesta av oss känner till idag. Ganska
      nyligen dock har människan börjat para in nya vildkattssorter, t.ex.
      asiatisk leopardkatt ("Felis Bengalensis") som blev Bengalen eller
      servalen ("Leptailurus serval") som blev Savannah. I takt med att fler
      människor skaffar dessa tamkatter och låter bli att kastrera dom kommer
      huskatterna att få fler vildkattsgener i sig. Då kommer också Européen
      skilja sig mer ifrån huskatterna vi känner till idag. Tittar man lite
      närmre har ju dagens huskatt säkert en bra blandning av andra raser (såsom
      perser, siames etc.) i släktskapet även om dom härstammar ifrån samma
      grund.
    </p>
    <h3>Varför en raskatt?</h3>
    <p>
      Det kanske inte säger så mycket att katterna kommer med en stamtavla men
      det betyder att vi har koll på deras släktskap och hälsa så att det inte
      skall dyka upp några obehagliga överaskningar. Självklart kan även
      raskatter bli sjuka men vi uppfödare gör vårt yttersta för att kolla upp
      innan att de inte bär på några ärftliga sjukdomar eller parar sig med sin
      kusin.
    </p>
    <p>
      Vi är anslutna till <a href="www.sverak.se">SVERAK</a> som i sin tur är
      anslutna till <a href="http://fifeweb.org/index.php">FIFe</a> vilket
      innebär att vi följer deras stadgar och regler. Det är en kvalitétsäkring
      att vi är seriösa uppfödare med god katthållning.
    </p>
    <h3>Varför så dyra?</h3>
    <p>
      Det kostar pengar att göra veterinärbesiktningar, DNA-tester och att hitta
      en bra hane. Det är väldigt få kattungeuppfödare som tjänar pengar på sin
      uppfödning. Det är också viktigt att komma ihåg att det inte är gratis att
      ha en katt och det är viktigt att ha med det i sin beräkning att
      försäkring, veterinärvård och mat kommer kosta på.
    </p>
    <p>
      I priset ingår även chipmärkning, försäkring, vaccination samt kastrering.
    </p>
    <p>
      Tyvärr finns det en attityd hos folk att det "bara" är att skaffa en
      kattunge för 10 kr hos närmsta bondgård vilket ger katten en status som
      "slit o släng" vara. Man kan skaffa en liten söt över sommaren för att
      sedan överge den när den börjar bli större och inte lika söt. (Alla
      tillhör såklart inte denna skaran utan för de allra flesta är katten är
      kär medlem i familjen.) Desto vanligare är kanske att många struntar i att
      kastrera sina katter vilket gör att det föds fler oönskade kullar /
      hemlösa katter. Det hela är en ond cirkel som både vi uppfödare och alla
      ideella kattföreningar jobbar emot. Idag är det lag på att du inte får
      släppa ut din okastrerade katt utan att ha koll på den, dvs inte låta den
      para sig okontrollerat. Köper du en en huskatt (katt utan stamtavla) från
      en privatperson sponsrar du ev. någon som bryter mot lagen.
    </p>
    <p>
      Vill du göra en god gärning finns det också helt fantastiskt fina katter
      som av olika anledningar saknar en matte eller husse på katthem runt om i
      Sverige. Där brukar en katt kosta runt 1000 kr.
    </p>
  </div>
</template>
